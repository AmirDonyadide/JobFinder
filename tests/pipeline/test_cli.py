...
def test_resolve_pipeline_profile_prefers_cli_and_supports_product_env():
    ...
    legacy = resolve_pipeline_profile(
        argparse.Namespace(profile=None),
        {"JOBFINDER_PROFILE": "phd"},
        default_profile="jobs",
    )
...
def test_main_resumes_incomplete_evaluation_without_scraping(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["jobfinder"])
